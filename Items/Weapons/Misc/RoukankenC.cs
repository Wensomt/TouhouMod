...
{
	public class RoukankenC : ModItem
	{
        private int count = 0;
		private int countB = 0;
		public override void SetStaticDefaults()
		{
			DisplayName.SetDefault("Roukanken C");
        ...
        public override bool Shoot(Player player, ref Vector2 position, ref float speedX, ref float speedY, ref int type, ref int damage, ref float knockBack)
		{
            ...
            if (player.altFunctionUse == 2)
            {
                ...
                if (countB % 1 == 0)
                {
                    Projectile.NewProjectile(position.X + speedY * 0.5f, position.Y + speedX * 0.5f, speedX, speedY, mod.ProjectileType("SlashS"), damage * 3 / 2, 0, player.whoAmI);
                    Projectile.NewProjectile(position.X - speedY * 0.5f, position.Y - speedX * 0.5f, speedX, speedY, mod.ProjectileType("SlashS"), damage * 3 / 2, 0, player.whoAmI);
                }
                return false;
		...
		public override void AddRecipes(){
			...
			recipe.AddIngredient(mod.ItemType("RoukankenB"));
