...
{
	public class RoukankenD : ModItem
	{
        private int count = 0;
		private int countB = 0;
		public override void SetStaticDefaults()
		{
			DisplayName.SetDefault("Roukanken D");
            ...
            else
            {
                ...
                if (count % 6 == 0)
                    ...
                    if (speedX > 0f)
                    {
                        ...
                        Projectile.NewProjectile(position.X + speedX * 8f, position.Y + speedY * 6f, speedX * 0.001f, speedY * 0.001f, mod.ProjectileType("Slash"), damage , 1, player.whoAmI);
                    }
                    else
                    {
                        Main.PlaySound(SoundLoader.customSoundType, -1, -1, mod.GetSoundSlot(SoundType.Custom, "Sounds/Custom/SLASH"), 0.5f);
                        ...
                        Projectile.NewProjectile(position.X + speedX * 4f, position.Y + speedY * 6f, speedX * 0.001f, speedY * 0.001f, mod.ProjectileType("Slash"), damage , 1, player.whoAmI);
                    }
                //float numberProjectiles = 2;
                //float rotation = MathHelper.ToRadians(32);
                //position += Vector2.Normalize(new Vector2(speedX, speedY)) * 45f;
		...
		public override void AddRecipes(){
			...
			recipe.AddIngredient(mod.ItemType("RoukankenC"));
