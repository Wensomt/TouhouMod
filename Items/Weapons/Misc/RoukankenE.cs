...
{
	public class RoukankenE : ModItem
	{
        private int count = 0;
		private int countB = 0;
		public override void SetStaticDefaults()
		{
			DisplayName.SetDefault("Roukanken E");
            ...
            else
            {
                ...
                if (count % 8 == 0)
                    ...
                    if (speedX > 0f)
                    {
                        ...
                        Projectile.NewProjectile(position.X + speedX * 4f, position.Y + speedY * 3f, speedX * 0.001f, speedY * 0.001f, mod.ProjectileType("Slash"), damage*3/2, 1, player.whoAmI);
                        Projectile.NewProjectile(position.X + speedX * 8f, position.Y + speedY * 6f, speedX * 0.001f, speedY * 0.001f, mod.ProjectileType("Slash"), damage*3/2, 1, player.whoAmI);
                    }
                    else
                    {
                        Main.PlaySound(SoundLoader.customSoundType, -1, -1, mod.GetSoundSlot(SoundType.Custom, "Sounds/Custom/SLASH"), 0.5f);
                        Projectile.NewProjectile(position.X + speedX, position.Y + speedY * 3f, speedX * 0.001f, speedY * 0.001f, mod.ProjectileType("Slash"), damage*3/2, 1, player.whoAmI);
                        Projectile.NewProjectile(position.X + speedX * 4f, position.Y + speedY * 6f, speedX * 0.001f, speedY * 0.001f, mod.ProjectileType("Slash"), damage*3/2, 1, player.whoAmI);
                    }
		...
		public override void AddRecipes(){
			...
			recipe.AddIngredient(mod.ItemType("RoukankenD"));
