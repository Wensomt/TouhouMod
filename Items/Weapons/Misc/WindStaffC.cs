...
{
	public class WindStaffC : ModItem
	{
        private int count = 0;
		private int countB = 0;
		public override void SetStaticDefaults()
		{
			DisplayName.SetDefault("Wind Maiden Staff C");
        ...
        public override bool Shoot(Player player, ref Vector2 position, ref float speedX, ref float speedY, ref int type, ref int damage, ref float knockBack)
		{
            ...
            if (player.altFunctionUse == 2)
            {
                ...
                if (countB % 2 == 0)
                {
						Projectile.NewProjectile(position.X - speedY / 3f, position.Y - speedX / 3f, speedX /2f, speedY / 2f, mod.ProjectileType("Snake"), damage/4*3, 1, player.whoAmI);
                        Projectile.NewProjectile(position.X + speedY / 3f, position.Y + speedX / 3f, speedX / 2f, speedY / 2f, mod.ProjectileType("Snake"), damage/4*3, 1, player.whoAmI);
                }
                return false;
            }
		...
		public override void AddRecipes(){
			...
			recipe.AddIngredient(mod.ItemType("WindStaffB"));
