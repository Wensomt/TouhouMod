...
{
	public class MarisaSigilE : ModProjectile
	{
        private int type = 0;
        private int timer = 0;
        ...
        public override void AI()
        {
            ...
            if (Main.netMode != 1)
            {
                if (timer % 11 == 0)
                {
                    ...
                    Projectile.NewProjectile(projectile.position.X + projectile.width / 2, projectile.position.Y + projectile.height / 2, projectile.velocity.X / 8f, projectile.velocity.Y / 8f, type, 24, 0f);
                }
            }
            projectile.velocity = projectile.velocity.RotatedBy(-16f / 120f);
        }
    }
}