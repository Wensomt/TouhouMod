...
{
    public class MasterSpark : ModProjectile
    {
		...
		{
			if (placement.X > projectile.position.X && placement.X < projectile.position.X + 40f && placement.Y > projectile.position.Y && placement.Y < projectile.position.Y + 40f)
				return true;
			return false;
		}
    }
